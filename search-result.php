...
$sql="SELECT * FROM `products` WHERE Name LIKE '%$Query%' OR Category LIKE '%$Query%' OR Info LIKE '%$Query%' LIMIT 10";
...
<script src="js/searchresultpagejs.js"></script>
...
echo'<div class="row search-page-items"><a href="product-page.php?ID='.$row["ID"].'"><img class="img-responsive img-rounded img-thumbnail col" src="'.$row["Path"].'" alt="'.$row["Name"].'"><label class="col-sm-2"><strong class="row">Name : '.$row["Name"].'</strong><div>Price : '.$row["Price"].'</div></label></a></div><hr>';

}
}
else{
echo "No related items found.";
}
?>
</div>
